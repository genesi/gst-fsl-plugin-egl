...
gst_gl_window_finalize (GObject * object)
{
  ...
  if(priv->gl_surface != EGL_NO_SURFACE)
    eglDestroySurface (priv->gl_display, priv->gl_surface);
...
gst_gl_window_new (gulong external_gl_context)
{
  ...
  XWMHints wm_hints;
  unsigned long mask;
  const gchar *title = "EGL X11 renderer";
  Atom wm_atoms[3];
...
void
gst_gl_window_set_draw_callback (GstGLWindow * window, GstGLWindowCB draw,
    GstGLWindowCB draw_finish, gpointer data)
{
  GstGLWindowPrivate *priv = window->priv;
  ...
  priv->draw_cb = draw;
  priv->draw_finish_cb = draw_finish;
  priv->draw_data = data;
    ...
    switch (event.type) {
      case ClientMessage:
      {
        ...
        if (wm_gl != None && event.xclient.message_type == wm_gl) {
          ...
          if (priv->running) {
            ...
            //g_print("========Call customer callback %p, arg %p\n", custom_cb, custom_data);
            custom_cb (custom_data);
          }
      ...
      case Expose:
        if (priv->draw_cb) {
          ...
          priv->draw_finish_cb(priv->draw_data);
        }
        break;
      ...
      case VisibilityNotify:
      {
        switch (event.xvisibility.state) {
          /*FIXME: For totem, it's not necessary because totem will call expose when window state changed
...
EGLDisplay
gst_gl_window_get_egl_display (GstGLWindow * window)
{
  EGLDisplay disp = NULL;
  if (window) {
	GstGLWindowPrivate *priv = window->priv;
    disp = priv->gl_display;
  }
  return disp;
}
